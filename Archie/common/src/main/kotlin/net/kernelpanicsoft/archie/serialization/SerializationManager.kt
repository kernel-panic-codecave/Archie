...
import net.kernelpanicsoft.archie.serialization.SerializationManager.SerializationManagerBuilder.DynamicOpRegistryBuilder
import net.kernelpanicsoft.archie.serialization.SerializationManager.SerializationManagerBuilder.SerializerRegistryBuilder
import net.kernelpanicsoft.archie.serialization.serializers.BuiltInSerializersModule
	...
	init {
		SerializationManager {
			...
			registerDynamicOp(NbtOps.INSTANCE) {
				encode { input, strategy -> nbt.encodeToNbtTag(strategy, input).toMinecraft }
				decode { input, strategy ->
					require(input is Tag) { "Expected input of type Tag but received ${input.javaClass.simpleName}." }

					nbt.decodeFromNbtTag(
						strategy,
						input.fromMinecraft ?: throw IllegalStateException("Failed to convert a Minecraft Tag into a KNbtTag.")
					)
				}
			}

			registerSerializer(JsonElement.serializer().descriptor, name = "JsonElement") {
				encode(JsonEncoder::class) { encoder, codec, input ->
					encoder.encodeJsonElement(codec.encodeStart(KOps.Json, input).orThrow)
				}
			...
			registerSerializer(NbtTag.serializer().descriptor, name = "NbtTag") {

...
abstract class ArchieModelProvider<T : ModelBuilder<T>>(
...
) : IArchieDataProvider
{
	...
	fun getBuilder(path: String, block: T.() -> Unit = {}): T
	{
		...
		val outputLoc =
			extendWithFolder(if (path.contains(":")) ResourceLocation(path) else ResourceLocation(mod.modId, path))
	...
	private fun extendWithFolder(rl: ResourceLocation): ResourceLocation
	{
		...
		return ResourceLocation(rl.namespace, folder + "/" + rl.path)
	}
	...
	fun nested(): T
	{
		return factory.apply(ResourceLocation("dummy:dummy"))
	}

	fun getExistingFile(path: ResourceLocation): ModelFile
	{
		val ret =
			ModelFile(

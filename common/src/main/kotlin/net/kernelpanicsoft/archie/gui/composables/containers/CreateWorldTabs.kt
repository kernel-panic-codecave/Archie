...
@Composable
fun CreateWorldTabs(
    tabs: List<CreateWorldTabSpec>,
    state: CreateWorldTabsState = rememberCreateWorldTabsState(tabs),
    modifier: Modifier = Modifier,
    onTabSelected: (CreateWorldTabSpec) -> Unit = {},
    tabSpacing: Int = 2,
        ...
        Row(
            ...
            tabs.forEach { tab ->
                CreateWorldTab(
                    ...
                    onClick = {
                        if (!tab.enabled) return@CreateWorldTab
                        state.select(tab.id)
                        onTabSelected(tab)
                    },
                )
...
data class CreateWorldTabSpec(
    val id: String,
    val title: Component,
    val icon: CreateWorldTabIcon? = null,
    val enabled: Boolean = true,
)

/** Sprite descriptor used for optional tab icons. */
data class CreateWorldTabIcon(
...
@Stable
class CreateWorldTabsState internal constructor(initialSelectedId: String?) {
    ...
    fun ensureSelection(tabs: List<CreateWorldTabSpec>) {
        if (tabs.isEmpty()) {
            selectedId = null
            return
        }
    ...
    fun selectedIndex(tabs: List<CreateWorldTabSpec>): Int =
        tabs.indexOfFirst { it.id == selectedId }

    fun selectedTab(tabs: List<CreateWorldTabSpec>): CreateWorldTabSpec? =
        tabs.firstOrNull { it.id == selectedId }
}

@Composable
fun rememberCreateWorldTabsState(
    tabs: List<CreateWorldTabSpec>,
    initialSelectedId: String? = tabs.firstOrNull { it.enabled }?.id,
): CreateWorldTabsState = remember(initialSelectedId) { CreateWorldTabsState(initialSelectedId) }

@Composable
fun CreateWorldTab(
    spec: CreateWorldTabSpec,
    selected: Boolean,
    modifier: Modifier = Modifier,
    ...
    onClick: (CreateWorldTabSpec) -> Unit,
) {
